...
namespace Consul.Provider.Grpc.Registrer
{
    public sealed class RegistrationProvider
    {
        ...
        public void RegisterGrpc(Uri serviceAddress, string? serviceId = null)
        {
            if (serviceAddress is null)
                throw new ArgumentNullException(nameof(serviceAddress));

            var instance = GetGrpcAgentServiceRegistration(serviceAddress, serviceId);
            Register(instance);
        }
        ...
        public void Register(AgentServiceRegistration instance)
        {
            ...
            {
                await _consulClient.Agent.ServiceDeregister(instance.ID);
                await _consulClient.Agent.ServiceRegister(instance);
            });
        ...
        private AgentServiceRegistration GetGrpcAgentServiceRegistration(Uri serviceAddress, string? serviceId = null)
        {
            if (serviceAddress is null)
                throw new ArgumentNullException(nameof(serviceAddress));
            ...
            var registrationInstance = new AgentServiceRegistration()
            {
                ...
                Meta = new Dictionary<string, string>() { ["Protocol"] = protocol },
                Tags = _consulConfig.Value.ServerTags,
                Check = AddGRPCHealthCheck($"{host}:{port}/Heath")
            };
            return registrationInstance;
        }
        ...
        /// <returns></returns>
        public AgentServiceCheck AddHttpHealthCheck(string url, int timeout = 10, int interval = 10)
        {
        ...
        /// <param name="interval">check interval. unit: second</param>
        /// <returns></returns>
        public AgentServiceCheck AddGRPCHealthCheck(string endpoint, bool grpcUseTls = false, int timeout = 10, int interval = 10)
        {
            return new AgentServiceCheck()
            {
                DeregisterCriticalServiceAfter = TimeSpan.FromSeconds(timeout * 3),
