...
namespace Enterprise.Core.Internal
{
    public class CapPublisher : ICapPublisher
    {
        ...
        protected static readonly DiagnosticListener s_diagnosticListener = new(DiagnosticListenerNames.DiagnosticListenerName);
        ...
        {
            if (s_diagnosticListener.IsEnabled(DiagnosticListenerNames.BeforePublishMessageStore))
            {
                ...
                s_diagnosticListener.Write(DiagnosticListenerNames.BeforePublishMessageStore, eventData);
        ...
        {
            if (tracingTimestamp != null && s_diagnosticListener.IsEnabled(DiagnosticListenerNames.AfterPublishMessageStore))
            {
                ...
                s_diagnosticListener.Write(DiagnosticListenerNames.AfterPublishMessageStore, eventData);
            }
        }

        private void TracingError(long? tracingTimestamp, Message message, Exception ex)
        {
            if (tracingTimestamp != null && s_diagnosticListener.IsEnabled(DiagnosticListenerNames.ErrorPublishMessageStore))
            {
                var now = DateTimeOffset.UtcNow.ToUnixTimeMilliseconds();
                var eventData = new CapEventDataPubStore()
                {
                ...
                s_diagnosticListener.Write(DiagnosticListenerNames.ErrorPublishMessageStore, eventData);
            }
        }
